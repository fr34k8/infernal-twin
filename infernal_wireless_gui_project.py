...
#~ 
		...
		read_only_txt = wx.TextCtrl(pnl, -1, 'Wireless Reconnaissance Result', style=wx.TE_MULTILINE|wx.TE_READONLY, pos=(20, 200),size=(500,600))
		...
		view_logs = wx.TextCtrl(pnl, -1, 'Logs are displayed here', style=wx.TE_MULTILINE|wx.TE_READONLY, pos=(680, 200),size=(500,600))
		#~ 
...
sh /usr/local/etc/raddb/certs/bootstrap'''
	...
	def sslStrip(self, e):
		try:
			...
			wx.MessageBox('There seem to be error with an interface\nMake sure you eth0 have Internet Access', 'Warning/Error', wx.ICON_ERROR | wx.ICON_INFORMATION)
...
class wpa_cracker(wx.Frame):
    ...
    def bruteforce(self, event):
		...
		test = subprocess.Popen(["aircrack-ng",self.capturelist,"-w",self.wordlist,"-e",str(mySSID.GetValue()),"-l","key_found.txt"], stdout=subprocess.PIPE)
...
class authorwindows(wx.Frame):
	def __init__(self, parent, title):
		wx.Frame.__init__(self, parent, -1, title)
		html = wx.html.HtmlWindow(self)
		html.SetPage("3nt0py<br>Email: 3ntr0py1337@gmail.com<br>LinkedIn: <a href='https://ae.linkedin.com/pub/mo-halilov/40/43a/5b3'> https://ae.linkedin.com/pub/mo-halilov/40/43a/5b3</a><br>")
